...
- **Spatial Discretization**: The environment is broken into a 2D Grid. Each cell typically represents a 0.5m x 0.5m area, which is roughly the shoulder width of a single human. This allows us to enforce mutual exclusion (two agents cannot occupy the same cell, preventing physically impossible overlapping).
...
The engine is written in Python, offering rapid iteration, excellent data structure support (dataclasses, typing), and a robust ecosystem for future data analysis (Pandas, Numpy). However, Python is not ideal for rendering 60FPS UI directly in a browser. Thus, the V3 architecture introduces a strict decoupled approach:
- **The Engine (Backend)**: Computes the physics, pathfinding, and states.
- **The API Layer**: Serves the state changes as JSON objects over REST/WebSockets via FastAPI.
