...
> 2. **Protocol:** Instructors must formally assign Student Assistants to be "Door Holders" during an emergency. The simulation showed that managed doors eliminate the 15-20% jam probability, smoothing the queue and preventing pile-ups.
...
> **Answer:** Absolutely. The engine was designed with a decoupled architecture. The core logic (`engine.py`) is completely independent of the visual UI. By simply altering the coordinate arrays in `grid.py` (defining new boundaries, exits, and obstacles), this exact engine can simulate the cafeteria, the library, or the auditorium.

## 2. Limitations

**Q: What is the biggest weakness of your simulation?**
> **Answer:** The biggest weakness is the lack of physical mass and shoving dynamics. Because it is a grid-based Cellular Automaton, an agent either occupies a cell or does not. In real life, three panicked people might wedge their physical bodies into a doorway, causing a structural arch that requires immense physical force to break. We abstract this using random "door jam" cooldown ticks, but we do not physically simulate the kinetic friction of bodies. 
