...
- **Reaction Time**: The `wait_until` and `packed_up` delay variables are based on psychological studies regarding "pre-movement time"—the time it takes for people to recognize an alarm, decide it's real, and gather belongings.
...
This phenomenon is known as the "Faster is Slower" effect in crowd dynamics. When everyone rushes the door simultaneously (high greed/panic), the arching effect occurs, and flow rate drops. V3 mathematically reproduces this without explicitly programming "get stuck at the door"—it emerges naturally from the Grid mutual exclusion and the Pathfinder's Greedy BFS.
