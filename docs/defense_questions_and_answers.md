...
> **Answer:** Engineers and architects design primarily for *daily utility, capacity, and spatial efficiency*, often ensuring the room just meets the minimum legal fire code for door widths. However, minimum fire codes do not account for behavioral psychology—specifically, the fact that students in a computer lab will universally attempt to retrieve their backpacks from the locker area before fleeing. Our simulation doesn't claim the engineers built an illegal room; it proves that the *interaction* between the furniture placement (lockers near the exit) and *human behavior* creates an unpredicted bottleneck that severely compromises safety.
...
> **Answer:** For macro-level crowd dynamics and pathfinding, continuous 3D physics engines are overly computationally expensive and introduce unnecessary complexities (like polygon clipping). A discrete grid-based ABM (where 1 cell = roughly 1 square meter) is the industry standard for egress modeling (similar to software like Pathfinder or FDS+Evac). It perfectly captures the critical metrics we need: spatial occupation, bottlenecks, and queueing theory, without the overhead of rendering 3D physics.

**Q: "Where did you get your data for agent speed, pack-up delays, and tripping probabilities?"**
> **Answer:** *(If you used literature)*: These parameters are inspired by established crowd-dynamics literature. For example, standard walking speed in egress models is often set around 1.0 to 1.5 m/s, which we map to our grid ticks. Pack-up delays reflect psychological "pre-movement times" observed in fire safety studies. *(If estimated)*: We used educated heuristic estimates to represent relative delays. The exact milliseconds matter less than the *distribution* of delays—some people react immediately, some hesitate, and some must pack up, creating realistic stagger in crowd flow.
...
> **Answer:** *(Panelists love it when you acknowledge your limitations gracefully)*. 
...
> 2. We assume all students are able-bodied; we did not introduce variables for students with physical mobility disabilities.
> 3. We assume a static fire fuel weight based on furniture. A real fire's spread is heavily influenced by HVAC systems and airflow, which our grid model does not calculate.
